...
const change=(e)=>{
  ...
  store.commit("setCollectionGroupList",temp)
}
onMounted(async () => {
  try {

    //获取已有收藏
    await axios.post('/collection/getCollectionGroup').then(resp=>{
      console.log("00===>")
      if(resp.code===200){
...
onMounted(async () => {
  try {
    ...
    //选中已有的选项
    await axios.post('/collection/getChoosedGroups',{
...
// 添加分组
const onSearch=()=>{
  axios.post('/collection/addCollectionGroup',{
